...
public class newLoan extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (cat == null) {
                HashSet<String> list = db.getSetOfCat();
                ...
                cat = cat.replaceAll("~", " ");
                List<Book> list = db.getListOfBooksByCategory(cat);
                bean.setListOfBooksByCategory(list);
                session.setAttribute("bean", bean);
                RequestDispatcher dis = request.getRequestDispatcher("newLoanTable.jsp");
                dis.forward(request, response);
            }
